...
const demoMode = process.env.DEMOMODE === 'true' ? true : false;
...
const decorateRecords = (records, mapping) => {
  ...
  records.forEach((record) => {
    ...
    if (isPrivateIP(record.data['srcaddr'])) {
      // return resolve({
      //   ip: ipAddress,
      //   continent_code: null,
      //   continent_name: null,
      ...
      //     offset: 0,
      //     is_dst: false,
      //     current_time: '2018-06-20 04:23:23.734+0000'
      //   }
      // });
      return;
    }
    ...
    let eniData = _.find(mapping, {
      'interfaceId': record.data['interface-id']
    });
    if (eniData) {
    ...
    promises.push(
      ...
      .then((geo) => {
        ...
        record.data['country_name'] = geo ? geo.country_name : ''
        record.data['isp'] = geo ? geo.isp : ''
        record.data['connection_type'] = geo ? geo.connection_type : ''
        record.data['organization'] = geo ? geo.organization : ''
        record.data['geoname_id'] = geo ? geo.geoname_id : ''
        record.data['continent_code'] = geo ? geo.continent_code : ''
        record.data['continent_name'] = geo ? geo.continent_name : ''
        record.data['country_code2'] = geo ? geo.country_code2 : ''
        record.data['country_code3'] = geo ? geo.country_code3 : ''
        record.data['country_capital'] = geo ? geo.country_capital : ''
        record.data['state_prov'] = geo ? geo.state_prov : ''

        //record.data['source-region-code'] = geo ? geo.region_code : ''
        //record.data['source-region-name'] = geo ? geo.region_name : ''
        record.data['city'] = geo ? geo.city : ''
